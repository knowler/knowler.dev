...
export async function post(c) {
	...
	if (!source) {
		issues.push({ field: "source", message: "Source URL is missing." });
	}
	if (!URL.canParse(source)) {
		issues.push({ field: "source", message: "Source URL is not a valid URL." });
	}
	if (!target) {
		issues.push({ field: "target", message: "Target URL is missing." });
	}
	...
	if (target && new URL(target).origin !== SITE_URL) {
		issues.push({
			field: "target",
			message: `Target URL must be for this website (i.e. ${SITE_URL}).`,
		});
	}
	if (source && target && source === target) {
		issues.push({
			form: true,
			message: "Source and target URLs cannot be the same.",
		});
	}
