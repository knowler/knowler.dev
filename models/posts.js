export class Posts {
	...
	get cache() {
		const cache = caches.open(`${Deno.env.get("DENO_DEPLOYMENT_ID")}.posts.${Deno.env.get("CONTENT_VERSION")}`);
	...
	async getBySlug(slug) {
		...
		response = new Response(new Blob([JSON.stringify(post)], { type: "application/json" }));
	...
	async store(post) {
		...
		return Promise.all([
			this.cache.put(this.slugURL(post.slug), new Response(blob)),
			this.cache.put(this.idURL(post.id), new Response(blob)),
		]);
	}
}
