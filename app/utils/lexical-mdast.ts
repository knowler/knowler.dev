  ...
  switch (mdastNode.type) {
    ...
    case "heading":
      ...
      if (mdastNode.children)
        heading.append(...mdastNode.children.map(mdastNodeToLexical));
      return heading;

    case "paragraph":
      const paragraph = $createParagraphNode();
      if (mdastNode.children)
        paragraph.append(...mdastNode.children.map(mdastNodeToLexical));
      return paragraph;
    ...
    case "text":
      return $createTextNode(mdastNode.value.replaceAll("\n", " "));

    case "code":
      const lines = mdastNode.value.split("\n");
      return $createCodeNode().append(
        ...lines.flatMap((line, index) => {
          const codeHighlightNode = $createCodeHighlightNode(
            line,
            mdastNode.lang
          );
...
function lexicalNodeToMdast(node: LexicalNode) {
  ...
  if ($isParagraphNode(node))
    return paragraph(node.getChildren().flatMap(lexicalNodeToMdast));
