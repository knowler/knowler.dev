    ...
    return json(
      {
        ...
        data: result?.errors?.robotName
          ? undefined
          : Object.fromEntries(await clonedRequest.formData()),
      },
      400
    );
  }
...
// @ts-ignore
const lister = new Intl.ListFormat("en", {
  style: "long",
  type: "conjunction",
});
  ...
  return (
    <Form ref={formRef} className="contact-form" method="post">
      ...
      <p className="privacy-disclosure">
        ...
        send an email to <a href={`mailto:${fallbackEmail}`}>{fallbackEmail}</a>{" "}
        (this is not my actual email, so don’t go adding it to your address book
        or anything).
      </p>
      ...
      {invalid ? (
        <div role="alert" className="field-error">
          {spam
            ...
            : `The following fields are invalid: ${lister.format(
                Object.keys(actionData.errors)
              )}. Please fix the issues and try again.`}
        </div>
      ) : null}
      {actionData?.success ? (
        <output className="formStatus">Message sent!</output>
      ) : null}
    </Form>
  );
}
